...
//		EditorGUI.PropertyField(position, property, label);
		...
		if (setProperty.UTF8String!= string.Empty) 
		{
			label.text = setProperty.UTF8String;
		}
		...
		else if (setProperty.IsDirty)
		{
			...
			FieldInfo field = type.GetField(setProperty.Name);
			if (pi == null && field == null)
			{
			...
			else
			{
				...
				if(pi!= null)
					...
					field.SetValue(parent,fieldInfo.GetValue(parent));
			}
